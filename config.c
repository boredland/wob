...
{
	if (strcmp(str, "compositor_default") == 0) {
		*value = WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT;
		return true;
	}
...
handler(void *user, const char *section, const char *name, const char *value)
{
	...
	if (strcmp(name, "width") == 0) {
		...
		if (parse_output_mode(value, &config->output_mode) == false) {
			wob_log_error("Invalid argument for output_mode. Valid options are compositor_default, whitelist and all.");
			return 0;
		}
	}
...
wob_config_init(struct wob_config *config)
{
	...
	config->output_mode = WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT;
