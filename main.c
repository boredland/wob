...
xdg_output_handle_done(void *data, struct zxdg_output_v1 *xdg_output)
{
	...
	wl_list_for_each (output_config, &app->wob_config.outputs, link) {
		if (strcmp(output->name, output_config->name) == 0 || app->wob_config.output_mode == WOB_OUTPUT_MODE_ALL) {
...
handle_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version)
{
	...
	else if (strcmp(interface, "wl_output") == 0 && app->wob_config.output_mode != WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT) {
...
wob_flush(struct wob *app)
{
	...
	enum wob_output_mode output_mode = app->wob_config.output_mode;

	if (output_mode == WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT) {
		wl_surface_attach(app->fallback_wob_surface->wl_surface, app->wl_buffer, 0, 0);
		wl_surface_damage(app->fallback_wob_surface->wl_surface, 0, 0, dimensions.width, dimensions.height);
...
wob_hide(struct wob *app)
{
	...
	if (output_mode == WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT) {
		wob_log_info("Hiding bar on compositor default output");
...
wob_show(struct wob *app)
{
	...
	if (output_mode == WOB_OUTPUT_MODE_COMPOSITOR_DEFAULT) {
		wob_log_info("Showing bar on compositor default output");
